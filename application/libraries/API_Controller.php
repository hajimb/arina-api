            ...
            if ($this->db->affected_rows() >= $limit_num)
            {
                ...
                if (isset($limit_time) AND !empty($limit_time))
                {
                    ...
                    if ($limit_time == 'everyday')
                    {
                        ...
                        $startDate = date('Y-m-d')."12:00 AM";
                        $endDate = date('Y-m-d h:i A', strtotime($startDate . ' +1 day'));
                        // $endDate = date('Y-m-d')."11:59 PM";
                        // echo DateTime::createFromFormat('Y-m-d h:i A', $t1date."12:00 AM")->format('U')."\n";
                        $start_date_timestamp= DateTime::createFromFormat('Y-m-d h:i A', $startDate)->format('U');
                        $end_date_timestamp = DateTime::createFromFormat('Y-m-d h:i A', $endDate)->format('U');
